      ...
      <section className="mb-8 bg-gray-800 bg-opacity-70 rounded-lg p-6 shadow-lg">
        ...
        {activeTab === 'jobs' && (
          <ul className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {jobs.map((job, idx) => (
              <li key={idx} className="bg-blue-900 bg-opacity-70 p-6 rounded-lg shadow-lg flex flex-col justify-between">
                <div>
                  <h3 className="text-xl font-bold mb-2">{job.title}</h3>
                  <p className="text-sm text-gray-300 mb-4">{job.deadline}</p>
                </div>
                <button className="self-start px-5 py-2 bg-green-600 rounded hover:bg-green-700 transition-colors font-semibold shadow-md">
                  Apply
                </button>
              </li>
            ))}
